...
const Header = () => {
  ...
  if (loading) {
    return (
      <div
        className={
          "fixed top-0 w-full h-[56px] shadow-md bg-blue-200 flex align-middle justify-center z-10"
        }
      >
  ...
  return (
    <>
      <div
        className={
          "fixed top-0 w-full h-[56px] shadow-md bg-blue-300 flex z-10 align-middle justify-center"
        }
      >
        <div className={"relative flex items-center w-3/4 xl:min-w-[1000px] "}>
          <div className="flex-none">
            ...
            <nav className="hidden md:block">
              <ul className={"flex"}>
                {categories.categories.map((el) => {
                  return (
                    <li key={el.urlSlug} className={"mr-2 p-[5px]"}>
                      <Link href={`/category/${el.urlSlug}/page/1`} passHref>
                        <a className={"font-semibold text-xl hover:text-white"}>
                          {el.name}
                        </a>
                      </Link>
                    </li>
